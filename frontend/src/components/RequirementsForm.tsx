    ...
    <form onSubmit={handleSubmit} className="space-y-6 w-full max-w-5xl">
      <div>
        ...
        <input
          ...
          className="mt-1 block w-full rounded-md py-5 bg-gray-800 border-gray-700 text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 px-4 text-sm"
          required
        />
      </div>
      <div>
        ...
        {requirements.objectives.map((objective, index) => (
          <div className="flex items-center justify-center">
            <input
              ...
              className="mt-1 block w-full rounded-md bg-gray-800 border-gray-700 text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 mb-2 py-1 px-4 text-sm"
            ...
            >
              <Trash2 className="w-4 h-4  hover:scale-[1.2]" />
            </button>
          </div>
        ))}
        {requirements.objectives.length < 4 && (
          <button
            type="button"
      ...
      <button
        ...
        className="w-full relative inline-flex items-center justify-center p-2 overflow-hidden font-medium text-blue-100
           bg-[#2F4F81] border border-transparent rounded-md shadow-sm transition duration-300 
          ease-out group focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-700
          active:scale-[0.9]"
      >
        <span className="absolute inset-0 flex items-center justify-center w-full h-full bg-[#2F4F81] transform -translate-x-full transition duration-300 ease-out group-hover:translate-x-0">
          <svg
            className="w-6 h-6"
            fill="none"
            stroke="currentColor"
        ...
        <span className="absolute flex items-center justify-center w-full h-full transition-transform duration-300 transform group-hover:translate-x-full">
          <Send className="w-4 h-4 mr-2" />
          Submit Requirements
        </span>
